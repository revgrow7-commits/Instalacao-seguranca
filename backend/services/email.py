...
def send_relatorio_visita_concluida(visita: dict) -> list[str]:
    """Envia relatório completo da VT concluída ao vendedor e/ou instalador.

    Retorna lista de emails para os quais o envio foi bem-sucedido.
    """
    if not RESEND_API_KEY:
        logger.warning("RESEND_API_KEY não configurado — email não enviado")
        return []
    ...
    total_km     = (float(km_ida) + float(km_volta)) * float(valor_km)
    aprovacao    = visita.get("aprovacao_status") or "PENDENTE"
    obs_admin    = visita.get("observacoes_admin") or "—"
    descricao    = visita.get("relatorio_descricao") or "—"
    situacao     = visita.get("relatorio_situacao") or "—"
    visita_id    = visita.get("id", "")
    numero_vt    = visita.get("numero_vt") or visita_id[:8].upper()
    link         = (f"https://instal-visual.com.br/visitas-tecnicas/{visita_id}"
                    if visita_id else "https://instal-visual.com.br")
    ...
    rows = "".join([
        ...
        _row("KM Volta",       f"{km_volta} km"),
        _row("Total a pagar",  f"R$ {total_km:.2f}", accent=True),
        _section("✅", "Resultado da Visita"),
        _row("Status de aprovação", aprovacao),
        _row("Situação reportada",  situacao),
        _row("Descrição",           descricao),
        _row("Observações admin",   obs_admin),
    ])
    ...
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:600px;
      ...
      <div style="background:linear-gradient(135deg,#16a34a 0%,#15803d 100%);padding:24px 28px">
        <p style="margin:0 0 2px;font-size:11px;font-weight:700;letter-spacing:.12em;
                  color:rgba(255,255,255,0.6);text-transform:uppercase">Indústria Visual</p>
        <h2 style="color:#fff;margin:0;font-size:22px;font-weight:700">✅ Relatório de Visita Técnica</h2>
        <p style="color:rgba(255,255,255,0.75);margin:6px 0 0;font-size:13px">
          VT&nbsp;#{numero_vt} &middot; {branch} &middot; {client_name}
        </p>
      </div>
      <div style="padding:24px 28px">
        <table style="width:100%;border-collapse:collapse">{rows}</table>
        <div style="margin-top:28px;text-align:center">
          <a href="{link}"
             style="display:inline-block;background:#16a34a;color:#fff;padding:12px 32px;
                    border-radius:8px;text-decoration:none;font-size:14px;font-weight:600">
            Ver Visita Técnica →
          </a>
        </div>
    ...
    subject = f"Relatório de Visita Técnica — {client_name} ({branch})"

    recipients: list[str] = []
    if visita.get("vendedor_email"):
        recipients.append(visita["vendedor_email"])
    if visita.get("installer_email"):
        email = visita["installer_email"]
        if email not in recipients:
            recipients.append(email)

    if not recipients:
        logger.warning("Nenhum destinatário para relatório de VT %s", visita_id)
        return []
    ...
    for addr in recipients:
        try:
            ...
            logger.info("Relatório VT %s → %s", numero_vt, addr)
            sent.append(addr)
        except Exception as exc:
            logger.error("Erro ao enviar relatório VT %s para %s: %s", numero_vt, addr, exc)
