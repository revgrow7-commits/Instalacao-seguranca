...
async def export_visitas_excel(
    ...
    """
    Gera planilha Excel com visitas técnicas filtradas.
    Colunas: Nº VT | Cliente | Endereço | Filial | Instalador ID | Data Agendada |
             KM Rodados | Valor/KM | Total (R$) | Status
    """
    ...
    headers = [
        ...
        "Data Agendada", "KM Rodados", "Valor/KM", "Total (R$)", "Status",
    ]
    ...
    for row_num, doc in enumerate(docs, start=2):
        ...
        ws.cell(row=row_num, column=1, value=doc.get("numero_vt") or "").border = border
        ws.cell(row=row_num, column=2, value=doc.get("client_name", "")).border = border
        ws.cell(row=row_num, column=3, value=doc.get("client_address", "")).border = border
        ws.cell(row=row_num, column=4, value=doc.get("branch", "")).border = border
        ws.cell(row=row_num, column=5, value=doc.get("installer_id") or "").border = border
        ws.cell(row=row_num, column=6, value=scheduled_raw or "").border = border
        ws.cell(row=row_num, column=7, value=doc.get("km_rodados") or "").border = border
        ws.cell(row=row_num, column=8, value=doc.get("valor_por_km", 1.50)).border = border
        ws.cell(row=row_num, column=9, value=doc.get("valor_total") or "").border = border
        ws.cell(row=row_num, column=10, value=doc.get("status", "")).border = border

    column_widths = {
        "A": 14, "B": 30, "C": 35, "D": 12, "E": 36,
        "F": 18, "G": 14, "H": 12, "I": 14, "J": 14,
    }
