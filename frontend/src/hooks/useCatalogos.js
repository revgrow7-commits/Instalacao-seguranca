...
export function useCatalogos() {
  ...
  useEffect(() => {
    ...
    api.listTiposServico()
      .then(r => setTiposServico((r.data || []).map(v => ({ value: v.nome, label: v.nome }))))
      .catch(err => console.error('useCatalogos: falha ao carregar tipos de serviço', err));
    api.listFerramentas()
      .then(r => setFerramentas((r.data || []).map(v => ({ value: v.nome, label: v.nome }))))
      .catch(err => console.error('useCatalogos: falha ao carregar ferramentas', err));
  }, []);
